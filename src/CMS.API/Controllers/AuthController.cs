...
public class AuthController : ControllerBase
{
    ...
    /// <summary>
    /// Verifies the credentials against AppUser (UserId match, IsActive = 1, SHA-256 password hash)
    /// and returns the user profile plus a signed 24h JWT. Any failed check returns a single generic
    /// 401 so the caller cannot tell which part was wrong.
    /// </summary>
    [AllowAnonymous]
    [HttpPost("login")]
    public async Task<ActionResult<LoginResponse>> Login([FromBody] LoginRequest request, CancellationToken ct)
    {
        ...
        var ok = user is not null
                 && user.IsActive
                 && string.Equals(user.PasswordHash, suppliedHash, StringComparison.OrdinalIgnoreCase);
        if (!ok)
    ...
    /// hash, enforces new-password complexity, requires new == confirm, then stores SHA256(new) and
    /// bumps PasswordUpdatedTime. No password or hash is returned. The user is taken from the JWT.
    /// </summary>
    [Authorize]
    ...
    public async Task<ActionResult> ChangePassword([FromBody] ChangePasswordRequest request, CancellationToken ct)
    {
        ...
        var currentHash = PasswordHasher.Sha256Hex(request.CurrentPassword ?? "");
        ...
        // 4. Store the new hash + timestamp.
        await _repository.ChangePasswordAsync(userId, PasswordHasher.Sha256Hex(request.NewPassword), ct);
        return NoContent();
    }

    /// <summary>
    /// Resets a target user's password back to the SysConfig default. <b>Admin only</b> — enforced by
    /// the role claim, so a non-Admin caller gets 403. Reads the default at runtime, stores
    /// SHA256(default), bumps PasswordUpdatedTime. No password or hash is returned.
    /// </summary>
