...
**Password rule everywhere:** `PasswordHash = SHA256(password)` rendered as **lowercase hex**
(`Infrastructure/PasswordHasher.Sha256Hex`). A hash is **never** sent to or from the client.
...
- **`Infrastructure/PasswordHasher.Sha256Hex(string) → string`** — the one hashing function (login
  verify, change, reset, and AppUser create/reset all use it).
...
if any of: user missing, `IsActive != 1`, or `SHA256(password) != PasswordHash` — evaluated together so
the caller can't tell which failed. On success build a JWT:
...
1. `SHA256(currentPassword)` must equal the stored hash — else `400 { message: "目前密碼不正確。" }`,
   change nothing.
...
4. Store `PasswordHash = SHA256(newPassword)`, `PasswordUpdatedTime = now`. Return **204**.
...
at runtime, set `PasswordHash = SHA256(default)` and `PasswordUpdatedTime = now`. Returns **204**
(404 if the user doesn't exist). No password/hash in the response.
...
- **Unit** (`AuthControllerTests`, `Fakes/InMemoryAuthRepository` + `FakeSigningKeyProvider`): login happy
  ...
  body userId, rejects blank name, 401 without claim; change-password valid path (hash=SHA256(new) +
...
- **Integration** (`AuthorizationTests`, `WebApplicationFactory<Program>` + `ConfigureTestServices`
  ...
  reset sets `PasswordHash = SHA256(default)` + bumps the timestamp with no hash in the response.
